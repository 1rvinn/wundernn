"""
Solution file for the ENSEMBLE LSTMAttentionModel (Input Projection Version).

This version:
1. Loads all 5 'LSTMAttentionModel' models trained via K-Fold.
2. Uses the updated model architecture with Input Projection.
...
# --- Model Definition ---
# This MUST match the class from 'train_input_projection_kfold.py' exactly.
...
class PredictionModel:
    def __init__(self):
        ...
        """
        print("Initializing PredictionModel (ENSEMBLE, Input Projection)...")
        ...
        self.seq_len = 100
        self.num_layers = 2
        self.dropout = 0.1 # Dropout usually disabled during inference
        self.n_folds = 5
        
        # UPDATED: Matches the prefix in train_input_projection_kfold.py
        base_checkpoint_name = 'lstm_attention_raw_checkpoint_fold' 
        ...
        try:
            ...
            if not os.path.exists(first_model_path):
                # Fallback in case fold indexing started at 1 in filename logic, though code used 0
            ...
            # --- CRITICAL CHANGE: Inferring Input Size with Projection Layer ---
            # Old way: state_dict['lstm.weight_ih_l0'].shape[1] -> This is now 'hidden_size'
            # New way: Look at the first layer of input_projection
            
            # shape is (out_features, in_features) -> (hidden_size, input_size)
            ...
            for i in range(self.n_folds):
                # Check for 0-based indexing (fold_0.pt) which is what the provided code produces
