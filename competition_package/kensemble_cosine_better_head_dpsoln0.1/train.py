"""
Single Fold Training Script - LSTM with Attention + Input Projection.

- Intended for parallel training: Set 'fold_to_train' in Args to 0, 1, 2, 3, or 4.
- Uses K-Fold Cross Validation (5 Folds).
- Uses the LSTMAttentionModel with **Improvement C** (Input Projection).
...
class LSTMAttentionModel(nn.Module):
    def __init__(self, input_size, hidden_size=128, num_layers=2, dropout=0.1):
        ...
        self.input_projection = nn.Sequential(
            ...
            nn.ReLU() # GELU is also a good option here
        )
