"""
Solution file for the LSTMAttentionModel (Raw Data + Volatility FE).

This version:
1. Loads the 'LSTMAttentionModel' (with 2N -> N signature).
2. Loads the 'lstm_attention_raw_volatility_checkpoint.pt' checkpoint.
3. Does NOT use a StandardScaler.
4. **Performs 10-step Volatility Feature Engineering on the fly.**
...
class LSTMAttentionModel(nn.Module):
    def __init__(self, input_size: int, output_size: int, hidden_size=128, num_layers=2, dropout=0.1):
        ...
        # 1. Encoder (The LSTM)
        # Input size is 2*N (raw + volatility)
...
class PredictionModel:
    def __init__(self):
        ...
        """
        print("Initializing PredictionModel (LSTM Attention + Volatility FE)...")
        ...
        self.checkpoint_path = 'lstm_attention_raw_volatility_checkpoint.pt'
        ...
        # Buffer for the 2*N combined features, to feed the model
        self.feature_buffer = [] 
        # Buffer for the original N raw states, to *calculate* volatility
    ...
    def predict(self, data_point: DataPoint) -> np.ndarray | None:
        ...
        # We need 'vol_window_size' samples to calculate std dev
        # e.g., if window_size=10, we need t=9 (which is the 10th step)
        ...
        combined_features = np.concatenate([current_state_np, current_vol])
        # --- END FE ---
