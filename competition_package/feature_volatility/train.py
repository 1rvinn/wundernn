...
- **MODIFIED: Adds rolling volatility features (std dev) to the input.**
...
# --- NEW: Helper function for volatility feature engineering ---
def add_volatility_features(df: pd.DataFrame, feature_cols: list, window_size: int) -> (pd.DataFrame, list):
    ...
    for col in feature_cols:
        ...
        # Calculate rolling std within each group, fill the first NaNs with 0
        # --- This is the fix ---
...
class SequenceDataset(Dataset):
    ...
    MODIFIED:
    - `all_feature_cols`: List of cols to use for the INPUT window (e.g., raw + volatility)
    ...
    def __init__(self, df: pd.DataFrame, all_feature_cols: list, target_cols: list, seq_len: int = 100, mode: str = "train"):
        ...
        for seq_ix, g in grouped:
            ...
            for t in range(len(g)):
                ...
                window = input_states[start : t + 1] # This data is (raw + volatility)
...
def main(args):
    ...
    df, all_input_cols = add_volatility_features(df, original_feature_cols, args.vol_window_size)
...
if __name__ == '__main__':
    class Args:
        def __init__(self):
            ...
            # --- NEW: Hyperparameter for volatility window ---
            self.vol_window_size = 10
            
            self.checkpoint = 'lstm_attention_raw_volatility_checkpoint.pt' # Renamed
