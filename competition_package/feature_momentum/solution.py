"""
Solution file for the LSTMAttentionModel (Raw Data + Momentum FE).

This version:
1. Loads the 'LSTMAttentionModel' (with 2N -> N signature).
2. Loads the 'lstm_attention_raw_momentum_checkpoint.pt' checkpoint.
3. Does NOT use a StandardScaler.
4. **Performs 1-step momentum Feature Engineering on the fly.**
5. Manages a 150-step "state buffer" of the (state + momentum) features.
"""
...
class LSTMAttentionModel(nn.Module):
    def __init__(self, input_size: int, output_size: int, hidden_size=128, num_layers=2, dropout=0.1):
        ...
        # 1. Encoder (The LSTM)
        # Input size is 2*N (raw + momentum)
...
class PredictionModel:
    def __init__(self):
        ...
        """
        print("Initializing PredictionModel (LSTM Attention + Momentum FE)...")
        ...
        self.num_layers = 3
        self.dropout = 0.1
        self.checkpoint_path = 'lstm_attention_raw_momentum_checkpoint.pt'
        ...
        # This buffer will store the (state + momentum) features
        self.state_buffer = [] 
        # We need to store the last state to calculate the *next* momentum
        self.last_state_np = None 
    ...
    def predict(self, data_point: DataPoint) -> np.ndarray | None:
        ...
        if self.last_state_np is None:
            # This is the first step (t=0)
            # Momentum is 0, just like fillna(0.0) in training
            current_momentum_np = np.zeros_like(current_state_np)
        else:
            # Calculate the 1-step momentum
            current_momentum_np = current_state_np - self.last_state_np
            
        # Combine into the (2*N) feature vector
        combined_features = np.concatenate([current_state_np, current_momentum_np])
        
        # Store the current state for the *next* step's calculation
        self.last_state_np = current_state_np
        # --- END FE ---
