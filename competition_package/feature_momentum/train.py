...
- **MODIFIED: Adds 1-step momentum features (x(t) - x(t-1)) to the input.**
...
def add_momentum_features(df: pd.DataFrame, feature_cols: list) -> (pd.DataFrame, list):
    ...
    for col in feature_cols:
        ...
        # Calculate diff within each group, fill the first NaN with 0
        df_out[mom_col_name] = grouped[col].diff(periods=1).fillna(0)
...
class SequenceDataset(Dataset):
    ...
    MODIFIED:
    - `all_feature_cols`: List of cols to use for the INPUT window (e.g., raw + momentum)
    ...
    def __init__(self, df: pd.DataFrame, all_feature_cols: list, target_cols: list, seq_len: int = 100, mode: str = "train"):
        ...
        for seq_ix, g in grouped:
            ...
            for t in range(len(g)):
                ...
                # Target is the RAW ABSOLUTE state (from target_states)
                target = target_states[t + 1]

                start = max(0, t - self.seq_len + 1)
                window = input_states[start : t + 1] # This data is (raw + momentum)
                
                if window.shape[0] < self.seq_len:
                    pad_len = self.seq_len - window.shape[0]
                    # Pad with zeros. Shape is (pad_len, num_all_features)
...
# --- MODIFIED MODEL: Accepts input_size and output_size ---
class LSTMAttentionModel(nn.Module):
    def __init__(self, input_size: int, output_size: int, hidden_size=128, num_layers=2, dropout=0.1):
        ...
        # 1. Encoder (The LSTM)
        # Input size is num_features + num_momentum_features
        ...
        # Output size is just num_features
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for x, y in dataloader: # y is raw absolute target (orig_features,)
            x, y = x.to(device), y.to(device)

            # Model predicts the raw absolute state
            pred = model(x) # pred is (orig_features,)
    ...
    r2s = []
    for i in range(y_true.shape[1]): # This is range(orig_features)
        try:
...
def main(args):
    ...
    print(f"Original features: {len(original_feature_cols)}, Old input size: {len(df.columns)}")
    print("Adding momentum features...")
    df, all_input_cols = add_momentum_features(df, original_feature_cols)
    ...
    # --- MODIFIED: Pass feature lists to Dataset ---
    ...
    # --- MODIFIED: Instantiate LSTMAttentionModel with input/output size ---
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        running_loss = 0.0
        for x, y in train_loader: # y is (orig_features,)
            x, y = x.to(device), y.to(device) # x is (all_features,)
            optimizer.zero_grad()
            out = model(x) # out is (orig_features,)
...
if __name__ == '__main__':
    class Args:
        def __init__(self):
            ...
            self.epochs = 20
            self.val_frac = 0.1
            self.checkpoint = 'lstm_attention_raw_momentum_checkpoint.pt'
