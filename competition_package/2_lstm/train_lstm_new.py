...
This is a lightweight starter script; you'll want to tune model size, training
schedule, and data augmentation for production.
"""
...
class SequenceDataset(Dataset):
    ...
    For each sequence, we produce many training examples: for each time t where
    need_prediction is True, input is previous states up to t (truncated/padded
    ...
    def __init__(self, df: pd.DataFrame, seq_len: int = 100, mode: str = "train", predict_delta: bool = False):
        ...
        for seq_ix, g in grouped:
            ...
            # For each timestep where need_prediction is True, create an example
            for t in range(len(g)):
                ...
                if self.predict_delta:
                    # target is the delta between next and current step
                ...
                self.examples.append((window, target)) # self.examples is a list of tuples
...
def evaluate(model, dataloader, device, predict_delta=False):
    ...
    with torch.no_grad():
        for x, y in dataloader:
            ...
            if predict_delta:
                # If predicting deltas, the "true" prediction is the last input state + the predicted delta.
                # The target `y` is the true delta, so we must compare `pred` against `y`.
                # To get the final state prediction, we add the predicted delta to the last input state.
                last_input_state = x[:, -1, :]
                final_state_pred = last_input_state + pred
                # The true final state is the last input state + the true delta (`y`).
                final_state_true = last_input_state + y
                
                ys.append(final_state_true.cpu().numpy())
                yps.append(final_state_pred.cpu().numpy())
            else:
                # If not predicting deltas, `y` is the true next state and `pred` is the predicted next state.
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_mean_r2 > best_val:
            ...
            # save full metadata with the checkpoint so inference can reconstruct preprocessing
            torch.save({
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, default='lstm_checkpoint.pt')
