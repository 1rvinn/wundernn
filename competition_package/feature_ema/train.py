...
- **MODIFIED: Adds Exponential Moving Average (EMA) features to the input.**
...
# --- NEW: Helper function for EMA feature engineering ---
def add_ema_features(df: pd.DataFrame, feature_cols: list, alphas_list: list) -> (pd.DataFrame, list):
    """
    Calculates EMAs for each feature with its corresponding alpha, grouped by sequence.
    """
    if len(feature_cols) != len(alphas_list):
        raise ValueError(f"Length of feature_cols ({len(feature_cols)}) must match length of alphas_list ({len(alphas_list)})")
        
    df_out = df.copy()
    grouped = df_out.groupby('seq_ix')
    
    new_cols = []
    for col, alpha in zip(feature_cols, alphas_list):
        ema_col_name = f"{col}_ema_a{str(alpha).replace('.', 'p')}"
        
        # Calculate EMA within each group.
        # adjust=False uses the standard recursive formula: EMA_t = alpha*X_t + (1-alpha)*EMA_t-1
        df_out[ema_col_name] = grouped[col].transform(
            lambda x: x.ewm(alpha=alpha, adjust=False).mean()
        )
        new_cols.append(ema_col_name)
        
    all_feature_cols = feature_cols + new_cols
...
class SequenceDataset(Dataset):
    ...
    - `all_feature_cols`: List of cols to use for the INPUT window (e.g., raw + EMA)
...
def main(args):
    ...
    # --- IMPORTANT: DEFINE YOUR ALPHAS LIST HERE ---
    # The list 'alphas_list' MUST have the same length as 'original_feature_cols'.
    
    # This is a placeholder that cycles 3 alphas. 
    # YOU SHOULD REPLACE THIS with your specific list.
    # Example: alphas_list = [0.1, 0.5, 0.3, 0.1, ... , 0.9]
    
    common_alphas = [0.5, 0.2, 0.05] # Placeholder (short, medium, long)
    alphas_list = [0.08, 0.05, 0.11, 0.05, 0.03, 0.07, 0.03, 0.08, 0.04, 0.03,
               0.04, 0.06, 0.04, 0.08, 0.11, 0.04, 0.05, 0.05, 0.08, 0.09,
               0.11, 0.03, 0.05, 0.06, 0.05, 0.05, 0.08, 0.05, 0.07, 0.02,
               0.05, 0.09] 
    
    if len(alphas_list) != n_features:
        raise ValueError(f"Custom alphas_list length ({len(alphas_list)}) does not match n_features ({n_features})")
    # ---
    
    print("Adding EMA features...")
    df, all_input_cols = add_ema_features(df, original_feature_cols, alphas_list)
    
    input_size = len(all_input_cols)
    output_size = len(target_cols)
    print(f"New input size: {input_size}, Target output size: {output_size}")
    # ---
...
if __name__ == '__main__':
    class Args:
        def __init__(self):
            ...
            self.epochs = 20
            self.val_frac = 0.1
            self.checkpoint = 'lstm_attention_raw_ema_checkpoint.pt' # Renamed
